...
const LoginPage = () => {
  ...
  const onSave = useCallback(() => {
    patchUserMetadata.fn(userInfo.user_id, metadata);
  }, [metadata]);
  ...
  return (
    <Layout>
      ...
      {silentAuth.running && <p>Authenticating...</p>}

      {!silentAuth.running && !userInfo && (
        <>
          <h2>Logged out</h2>
      ...
      {!silentAuth.running && userInfo && (
        <>
          ...
          <button onClick={onSave} disabled={patchUserMetadata.running}>
            {patchUserMetadata.running ? "Saving..." : "Save"}
          </button>
